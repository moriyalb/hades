 ...
 */
pro.addProxy = function (record, app) {
	if (!record) {
		return;
	}
	var proxy = generateProxy(this, record, this._context, app);
	if (!proxy) {
		return;
	}
 ...
 */
pro.addProxies = function (records, app) {
	if (!records || !records.length) {
		return;
	}
	for (var i = 0, l = records.length; i < l; i++) {
		this.addProxy(records[i], app);
	}
};
...
pro.addServers = function (servers) {
	//console.log("client::add servers -> ", servers)
...
pro.rpcInvoke = function (serverId, msg, cb) {
	//console.log("rpcInvoke ---------------", serverId, msg)
 ...
 * @api private
 */
var generateProxy = function (client, record, context, app) {
	if (!record) {
		return;
	}
	var res, name;
	if (record.namespace == "user") {
		...
		modules = Hades.LoaderUtil.load(record.path, context);
		if (modules) {
			res = {};
			for (name in modules) {
...
var rpcToSpecifiedServer = function (client, msg, serverType, serverId, cb) {	
	if (typeof serverId !== 'string') {
		Logger.error('[pomelo-rpc] serverId is not a string : ', serverType, serverId);
		return;
	}
	if (serverId === '*') {
