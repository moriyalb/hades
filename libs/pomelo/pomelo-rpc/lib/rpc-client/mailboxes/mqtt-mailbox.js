...
MailBox.prototype.connect = function (tracer, cb) {
	...
	this.socket.on('publish', function (pkg) {
		...
		try {
			//console.log("mqttmb->public-> ", pkg, self.serverId)
			pkg = JSON.parse(pkg);
	...
	this.socket.on('error', function (err) {
		Logger.error(`mqttmb socketerror ${err}, remote server ${self.serverId}`)
		self.emit('close', self.id);
	});
...
var processMsg = function (mailbox, pkg) {
	var pkgId = pkg.id;
	//console.log("mqmb->processMsg-> ", pkgId);
	...
	//console.log("on mqtt process msg");
