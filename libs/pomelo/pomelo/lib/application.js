 ...
 * @memberOf Application
 */
Application.loadConfigBaseApp = function (key) {	
	const env = Hades.Config.getEnv()
	if (key == Constants.RESERVED.SERVERS){		
		this.set(key, Hades.Config.serverCfg()[env])
	}else{
		this.set(key, Hades.Config.masterCfg()[env])
	}
}
