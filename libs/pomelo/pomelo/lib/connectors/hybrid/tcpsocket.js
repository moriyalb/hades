...
var ondata = function (socket, chunk) {
	...
	if (typeof chunk === 'string') {
		chunk = Buffer.alloc(chunk, 0, 'utf8');
	}
...
var readHead = function (socket, data, offset) {
	...
	if (socket.headOffset === socket.headSize) {
		...
		var size = socket.headHandler(socket.headBuffer, false);
		if (size < 0) {
...
var readBody = function (socket, data, offset) {
	...
	//console.log("on readBody -> ", socket.packageSize, dlen, blen)
...
var resetHead = function (socket, isLong) {
	if (isLong) {
		socket.headSize = 8
	} else {
		socket.headSize = 4
	}
	socket.headBuffer = Buffer.alloc(socket.headSize);
}
