...
SessionService.prototype.remove = function (sid) {
	...
	if (session) {
		...
		//console.log("remove session -> ", sid, uid, this.uidMap)
	}
...
SessionService.prototype.kick = function (uid, reason, cb) {
	...
	//console.log("session service kick -> ", uid, reason)
	...
	if (sessions) {
		...
		sids.forEach(function (sid) {
			if (!!self.sessions[sid]){
				self.sessions[sid].closedByReason(reason);
			}else{
				console.error("Invalid session ", uid, sids)
			}
		});
...
Session.prototype.closedByReason = function (reason) {
	...
	if (_.isBoolean(reason)){
		this.emit('closed', this.toFrontendSession(), "ClientQuit");
		...
		this.emit('closed', this.toFrontendSession(), "SocketGone");		
	}else{
		this.emit('closed', this.toFrontendSession(), "UnknownReason");	
		console.error("Unknown Reason Type -> ", reason)
	}
