...
var crashLogger = Logger;
var adminLogger = Logger;
var admin = Hades.Admin;
...
Server.prototype.start = function (cb) {
	...
	this.masterConsole.on('disconnect', function (id, type, info, reason) {
		if (type == "client") {
			console.log("GMTool Client Disconneted ", id, info, reason)
			return
		}
		crashLogger.error(util.format('[%s],[%s],[%s],[%s]', type, id, Date.now(), reason || 'disconnect'));
		var count = 0;
		var time = 0;
		var pingTimer = null;
