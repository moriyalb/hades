...
pro.run = function () {
	try {
		jobCount++;
		this.runTime++;
		if (this.excuteTime() > 0) {
			var late = Date.now() - this.excuteTime();
			if (late > warnLimit)
				console.warn('run Job count ' + jobCount + ' late :' + late + ' lateCount ' + (++lateCount) + " excuteTime " + this.excuteTime());
		} else {
			//避免Warning
		}
		this.func(this.data);
	} catch (e) {
		console.error("Job run error for exception ! " + e.stack);
	}
};
