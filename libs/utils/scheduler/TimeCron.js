...
class TimeCron {
	...
	isInCronTime(cronTime, time){
		// console.log('input cron time:', JSON.stringify(cronTime))
		let date = new Date(time)

		if (!cronTime){
			console.error('inCronTime crontime is invalid!')
			return false
		}
		...
		if (!this.timeMatch(date.getFullYear(), cronTime[this.YEAR])){
			// console.log('isInCronTime year missmatch!', date.getFullYear(), JSON.stringify(cronTime[this.YEAR]))
			return false
		}

		if (!this.timeMatch(date.getDay(), cronTime[this.DOW])){
			// console.log('isInCronTime day of week missmatch!', date.getDay(), JSON.stringify(cronTime[this.DOW]))
			return false
		}

		if (!this.timeMatch(date.getMonth(), cronTime[this.MONTH])){
			// console.log('isInCronTime month missmatch!', date.getMonth(), JSON.stringify(cronTime[this.MONTH]))
			return false
		}

		if (!this.timeMatch(date.getDate(), cronTime[this.DOM])){
			// console.log('isInCronTime date missmatch!', date.getDate(), JSON.stringify(cronTime[this.DOM]))
			return false
		}

		if (!this.timeMatch(date.getHours(), cronTime[this.HOUR])){
			// console.log('isInCronTime hour missmatch!', date.getHours(), JSON.stringify(cronTime[this.HOUR]))
			return false
		}

		if (!this.timeMatch(date.getMinutes(), cronTime[this.MIN])){
			// console.log('isInCronTime minutes missmatch!', date.getMinutes(), JSON.stringify(cronTime[this.MIN]))
			return false
		}

		if (!this.timeMatch(date.getSeconds(), cronTime[this.SECOND])){
			// console.log('isInCronTime second missmatch!', date.getSeconds(), JSON.stringify(cronTime[this.SECOND]))
			return false
		}

		// console.log('isInCronTime all match!')
		return true
	}
